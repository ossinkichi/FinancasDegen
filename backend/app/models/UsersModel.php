...
class UsersModel extends ConnectModel
{
  ...
  public function updateDataUser(object|array $data)
  {
    try {
      ...
      $sql = $this->db->prepare('UPDATE users SET hash = :hash name = :name, email = :email, password = :password, cpf = :cpf, dateofbirth = :dateofbirth, gender = :gender, phone = :phone WHERE id = :id OR userhash = :userhash');
